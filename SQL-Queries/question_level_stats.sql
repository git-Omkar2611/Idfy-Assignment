...
GROUP BY
    d_q.question_key, d_q.question_text
ORDER BY
...
SELECT
    d_q.question_key,
    d_q.question_text,
    COUNT(fqa.answer_key) AS total_answers_for_question,
    SUM(CASE WHEN fqa.is_correct = TRUE THEN 1 ELSE 0 END) AS total_correct_answers,
    (SUM(CASE WHEN fqa.is_correct = TRUE THEN 1 ELSE 0 END) * 100.0) / COUNT(fqa.answer_key) AS percentage_correct
FROM
    mcq_quiz_gold.fact_question_answer AS fqa
JOIN
    mcq_quiz_gold.dim_question AS d_q ON fqa.question_key = d_q.question_key
...
SELECT
    ...
    (SUM(CASE WHEN fqa.is_correct = FALSE THEN 1 ELSE 0 END) * 100.0) / COUNT(fqa.answer_key) AS percentage_incorrect
FROM
    mcq_quiz_gold.fact_question_answer AS fqa
JOIN
    mcq_quiz_gold.dim_question AS d_q ON fqa.question_key = d_q.question_key
